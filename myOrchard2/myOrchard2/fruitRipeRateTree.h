...
class AVL_Tree
{
	struct node
	{
		int fruitRipeRate=fruit.ripeRate;
		F* fruit;
		node* left;
		node* right;
		int height;
	...
	node* insert(int x, T* fruit, node* t)
	{
		...
		else if (x < t->fruitRipeRate)
		{
			t->left = insert(x, fruit, t->left);
			if (height(t->left) - height(t->right) == 2)
			{
				if (x < t->left->fruitRipeRate)
	...
	node* remove(int x, node* t)
	{
		...
		else if (t->left && t->right)
		{
			...
			t->fruitRipeRate = temp->fruitRipeRate;
			t->right = remove(t->fruitRipeRate, t->right);
		}
